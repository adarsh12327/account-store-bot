...
function server1CountriesMenu() {
  return Markup.inlineKeyboard([
    [Markup.button.callback("➕ Add Country", "admin:countries:server1:add")],
    [Markup.button.callback("📋 Country List", "admin:countries:server1:list")],
    [Markup.button.callback("🔎 Search Country", "admin:countries:server1:search")],
    [Markup.button.callback("⬅ Countries", "admin:countries")],
  ]);
}
function countryListKeyboard(countries) {
  const rows = countries.map((c) => [Markup.button.callback(`${c.emoji || "🌍"} ${c.name} (${c.status})`, `admin:countries:view:${c.id}`)]);
  rows.push([Markup.button.callback("⬅ Countries Menu", "admin:countries")]);
  return Markup.inlineKeyboard(rows);
}
function countryDetailKeyboard(country) {
  const toggle = country.status === "enabled"
...
function productListKeyboard(products, page = 1, total = products.length, search = "") {
  const PAGE_SIZE = 40;
  ...
  for (let i = 0; i < products.length; i += 2) {
    ...
    for (let j = i; j < Math.min(i + 2, products.length); j++) {
      ...
      const countryName = String(product.countryName || product.name || "")
        .replace(/^🌍\s*/u, "").replace(/Telegram\s*/i, "").trim();
      row.push(Markup.button.callback(`${countryName} — ₹${Number(product.finalPrice || 0).toFixed(2)}`, `admin:products:view:${product.id}`));
    }
    rows.push(row);
  }
  rows.push([Markup.button.callback("🔎 Search Product", "admin:products:search")]);
  const navigation = [];
  if (page > 1) navigation.push(Markup.button.callback("⬅️ Previous", `admin:products:page:${page - 1}${search ? `:${encodeURIComponent(search)}` : ""}`));
  if (page < totalPages) navigation.push(Markup.button.callback("Next ➡️", `admin:products:page:${page + 1}${search ? `:${encodeURIComponent(search)}` : ""}`));
  if (navigation.length) rows.push(navigation);
  rows.push([Markup.button.callback("🗑️ Delete All Products", "admin:products:deleteall")]);
  rows.push([Markup.button.callback("⬅️ Products Menu", "admin:products")]);
  return Markup.inlineKeyboard(rows);
}
function productDetailKeyboard(product) {
  const toggle = product.status === "enabled"
    ? Markup.button.callback("🔴 Disable", `admin:products:disable:${product.id}`)
    : Markup.button.callback("🟢 Enable", `admin:products:enable:${product.id}`);
  return Markup.inlineKeyboard([
    [Markup.button.callback("📝 Edit Name", `admin:products:editname:${product.id}`), Markup.button.callback("😀 Edit Emoji", `admin:products:editemoji:${product.id}`)],
    [Markup.button.callback("📄 Description", `admin:products:editdescription:${product.id}`), Markup.button.callback("📈 Profit Margin", `admin:products:margin:${product.id}`)],
    [Markup.button.callback("🔧 Service Code", `admin:products:editservice:${product.id}`), Markup.button.callback("🌍 Country", `admin:products:editcountry:${product.id}`)],
    [toggle, Markup.button.callback("❌ Delete", `admin:products:delete:${product.id}`)],
    [Markup.button.callback("⬅ Product List", "admin:products:list")],
  ]);
}
function countryPickerKeyboard(countries, callbackPrefix) {
  const rows = countries.map((c) => [Markup.button.callback(`${c.emoji || "🌍"} ${c.name}`, `${callbackPrefix}:${c.id}`)]);
...
function priceModeKeyboard(callbackPrefix) {
  return Markup.inlineKeyboard([
    [Markup.button.callback("MANUAL", `${callbackPrefix}:MANUAL`), Markup.button.callback("API", `${callbackPrefix}:API`)],
...
function userDetailKeyboard(user) {
  const banToggle = user.banned
    ? Markup.button.callback("✅ Unban", `admin:users:unban:${user.telegramId}`)
    : Markup.button.callback("🚫 Ban", `admin:users:ban:${user.telegramId}`);
  return Markup.inlineKeyboard([
    [Markup.button.callback("📥 Deposit History", `admin:users:deposits:${user.telegramId}:1`), Markup.button.callback("🛒 Order History", `admin:users:orders:${user.telegramId}:1`)],
    [Markup.button.callback("💰 Add Balance", `admin:users:addbal:${user.telegramId}`), Markup.button.callback("💸 Remove Balance", `admin:users:removebal:${user.telegramId}`)],
    [banToggle],
    [Markup.button.callback("👥 Referral Rate", `admin:users:referral:${user.telegramId}`)],
    [Markup.button.callback("⬅ Users List", "admin:users:list:1")],
  ]);
}

// ---------------- Deposits ----------------
function depositAdminKeyboard(depositId) {
  return Markup.inlineKeyboard([[Markup.button.callback("✅ Approve", `admin:deposits:approve:${depositId}`), Markup.button.callback("❌ Reject", `admin:deposits:reject:${depositId}`)]]);
}
...
function depositListKeyboard(deposits) {
  const rows = deposits.map((d) => [Markup.button.callback(`₹${d.amount} — ${d.userId}`, `admin:deposits:view:${d.depositId}`)]);
...
function server1OrderDetailMenu() {
  return Markup.inlineKeyboard([
    [Markup.button.callback("⬅ Processing Orders", "admin:server1:orders:processing")],
    [Markup.button.callback("⬅ Server 1 Orders", "admin:server1:orders")],
  ]);
}
...
function orderListKeyboard(orders) {
  const rows = orders.map((o) => [Markup.button.callback(`${o.productName} — ${o.userId}`, `admin:orders:view:${o.orderId}`)]);
  rows.push([Markup.button.callback("⬅ Orders Menu", "admin:orders")]);
  return Markup.inlineKeyboard(rows);
}
function orderDetailKeyboard(order) {
  if (order.status !== "processing") return Markup.inlineKeyboard([[Markup.button.callback("⬅ Order List", "admin:orders:list")]]);
  return Markup.inlineKeyboard([
...
function settingsMenu(settings = {}) {
  ...
  return Markup.inlineKeyboard([
    [Markup.button.callback("💰 Pricing", "admin:settings:productPricing"), Markup.button.callback("📢 Force Join", "admin:settings:forceChannel")],
